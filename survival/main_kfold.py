...
def parse_root_mapping(value, arg_name):
    if value is None:
    ...
    if not isinstance(mapping, dict):
        raise TypeError(f"{arg_name} must be a JSON object or a path to a JSON file")
    return mapping
...
def main(args):
    ...
    # define dataset
    args.pt_roots = parse_root_mapping(args.pt_roots, "--pt_roots")
    args.h5_roots = parse_root_mapping(args.h5_roots, "--h5_roots")
