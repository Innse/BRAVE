    ...
    def learning(self, model, dataloaders, criterion, optimizer, scheduler):
        ...
        for epoch in range(self.epoch, self.args.num_epoch):
            ...
            if val_results["Macro_AUC"] > self.results["val"]["Macro_AUC"]:
                ...
                self.args.run.log({f"best_{split}_Macro_AUC": self.results[split]["Macro_AUC"] for split in self.results.keys() if split not in ["train"]})
            ...
            print(">>>")
            print(">>>")
        self.args.run.log({f"final_best_{split}_Macro_AUC": self.results[split]["Macro_AUC"] for split in self.results.keys() if split not in ["train"]})
        return self.results
